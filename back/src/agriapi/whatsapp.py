...
def send_whatsapp(to_phone: str, body: str) -> bool:
    ...
    sid, token, sender = _cfg("TWILIO_ACCOUNT_SID"), _cfg("TWILIO_AUTH_TOKEN"), _cfg("TWILIO_WHATSAPP_FROM")
    if not (sid and token and sender):
        logger.warning("WhatsApp not configured (TWILIO_* missing) — skipping send to %s", to_phone)
        return False
