...
Strangler port of ``apps/assistant/orchestrator.py`` — the rule table + routing
logic are byte-identical to the Django original (same intents, reply keys, and
trigger vocabulary). The only framework difference is ``get_orchestrator``,
which reads the sidecar's ``AppSettings.ai_api_key`` instead of Django settings.
"""
...
_RULES: list[_IntentRule] = [
    ...
    _IntentRule(
        "irrigation_advice",
        "misc.chatbot.irrigationCard.intro",
        [
            "/irrigation",
            "/advice",
            "/conseil",
        ...
        tool="get_irrigation_advice",
    ),
    _IntentRule(
        "notifications",
        "misc.chatbot.notificationsCard.intro",
        [
            "/notifications",
            "/notifs",
            "notifications",
