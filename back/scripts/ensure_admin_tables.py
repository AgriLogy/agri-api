"""Idempotently create the business-admin schema (billing / audit / settings).

Schema-of-record lives in agri-db (Alembic) and Django doesn't run `migrate` on
boot, so — like the assistant + technician tables — these unmanaged tables are
created out-of-band here. Run on web boot (docker-entrypoint.sh): it creates any
of the five tables that are missing, so it's safe to run every start.
"""
...
TAG = "[ensure-admin-tables]"

MODELS = (Plan, Subscription, Invoice, AuditEvent, SystemSetting)
